...
dato un array di note, 
restituisce la prima nota non completata. 
Una nota viene considerata completata se tutti i 'todo' presenti hanno il flag 'done' impostato a 'true'.
*/

function firstUncompletedNote(records) {
  const firstNote = records.find(record => record.todos.some(todo => todo.done === false));
  return firstNote;
}
